...
/*******************************************************************************
_____loadOperationControls_____
Description
    Load Operation Control stuff
*******************************************************************************/
function loadOperationControls() {
  document.getElementById("previous-action-ten").addEventListener("click", () => { loadPreviousAction(10) })
...
function updateMessages() {
  let sap = stateActionPairs[actionIndex]
  actionMessage.innerText = `Action ${actionIndex}: ${stateActionPairs[actionIndex].action}`
  stateMessage.innerText = `HEALTH: ${sap.state.health}  ENERGY: ${sap.state.energyLevel}  LTS: ${sap.longTermScore}  STS: ${sap.shortTermScore}`
}

function loadPreviousAction(x) {
  let prevIndex = actionIndex
  actionIndex = Math.max(0, actionIndex - x)
  updateMessages()
  for (let i = actionIndex + 1; i < prevIndex + 1; i++) removeAction(i)
}
...
function loadNextAction(x) {
  let prevIndex = actionIndex
  actionIndex = Math.min(stateActionPairs.length - 1, actionIndex + x)
  updateMessages()
  for (let i = prevIndex + 1; i < actionIndex + 1; i++) addAction(i)
}
...
function addAction(x) {
  ...
  if (isMovementAction(sap.action)) {
    ...
    // add scan
    console.log("ADDING Scan", sap)

  }
}

function isMovementAction(action) {
  return (action == "north" || action == "south" || action == "west" || action == "east")
}
...
export {loadOperationVisualizer}
